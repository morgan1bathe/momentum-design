...
`mdc-menusection` groups related menu items under a labelled header inside a menu or menubar, so longer menus stay scannable. It also scopes single-selection for any `mdc-menuitemradio` children that share a `name`.
